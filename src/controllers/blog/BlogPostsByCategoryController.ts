...
interface GetPostsByCategoryQuery {
  page?: string;
  limit?: string;
...
export class BlogPostsByCategoryController {
  /**
   * Get all blog posts belonging to a specific category
   * Supports filtering, pagination, and search within the category
   */
  async getPostsByCategory(req: AuthRequest, res: Response) {
    try {
      const user = req.user!;
      ...
      const {
        ...
        startDate,
        endDate
      }: GetPostsByCategoryQuery = req.query;

      // FIXED: Validate pagination parameters with unlimited option
      ...
      // Get all post IDs that belong to this category
      const categoryPostAssociations = await BlogPostCategory.findAll({
        where: { categoryId: category.id },
        attributes: ['postId']
      });

      const categoryPostIds = categoryPostAssociations
      ...
      const whereConditions: any = {
        clientId,
        id: { [Op.in]: categoryPostIds } // Now guaranteed to be number[]
      };
  ...
  offset,
  attributes: ['id', 'uuid', 'title', 'slug', 'excerpt', 'content', 'status', 'isIndexable', 'authorId', 'featuredImageId', 'publishedAt', 'authorData', 'createdAt', 'updatedAt'] // Added 'authorData'
};
...
const formattedPosts = posts.map(post => {
  ...
  return {
    ...
    authorData: post.authorData, // Added authorData field
    author: author ? {
      id: author.id,
   ...
   * Useful for category listing pages
   */
  async getCategoryWithPostCount(req: AuthRequest, res: Response) {
    try {
      const user = req.user!;
      const clientId = user.clientId;
      const { slug: categorySlug } = req.params;

      if (!clientId || !categorySlug) {
        return res.status(400).json({
          success: false,
          message: 'Client ID and category slug are required'
        });
      }
      ...
      // Count posts in this category (published only)
      ...
      console.error('Error fetching category with post count:', error);
...
// Add proper exports
export default BlogPostsByCategoryController;